...
def align_weekly_returns(stock_points, index_points) -> pd.DataFrame:
    """주간 종가 -> 같은 일자 inner join 후 로그수익률."""
...
def compute_beta(
    ...
    *,
    ...
    span_days = (used_returns.index.max() - used_returns.index.min()).days
    listing_years = span_days / 365.25 if span_days > 0 else 0.0
    if listing_years < val_cfg["min_listing_years"]:
    ...
    return BetaResult(
        ...
        listing_years=round(listing_years, 3),
        warnings=warnings,
