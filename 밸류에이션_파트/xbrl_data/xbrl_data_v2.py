...
def get_corp_code(name_or_code: str, api_key: str | None = None) -> dict:
    ...
    if query.isdigit():
        ...
        if len(hit) > 1:
            ...
            print(f"      -> 실제 사업보고서 존재 여부로 자동 선별합니다.")

            # 최근 3년 구간에서 사업보고서 공시 여부 확인
            import datetime
            cur_year = datetime.date.today().year
            selected_row = None
            for _, cand_row in hit.iterrows():
                try:
                    chk_params = {
                        "crtfc_key": key,
                        "corp_code": cand_row["corp_code"],
                        "pblntf_ty": "A",
                        "bgn_de": f"{cur_year - 3}0101",
                        "end_de": f"{cur_year + 1}1231",
                        "page_count": "10",
                    }
                    chk_res = requests.get(
                        f"{DART_BASE}/list.json", params=chk_params, timeout=30
                    )
                    chk_data = chk_res.json()
                    chk_items = chk_data.get("list", [])
                    has_annual = any(
                        "사업보고서" in it.get("report_nm", "") for it in chk_items
                    )
                    if has_annual:
                        selected_row = cand_row
                        print(f"      -> '{cand_row['corp_name']}' ({cand_row['stock_code']}) "
                              f"사업보고서 확인됨. 해당 기업을 사용합니다.")
                        break
                except Exception:
                    continue

            if selected_row is None:
                # 사업보고서 확인 실패 시 첫 번째 fallback
                selected_row = hit.iloc[0]
                print(f"      -> 사업보고서 확인 실패. 첫 번째 "
                      f"'{selected_row['corp_name']}' 를 사용합니다.")
            return {
                "corp_code": selected_row["corp_code"],
                "corp_name": selected_row["corp_name"],
                "stock_code": selected_row["stock_code"],
            }
...
def get_rcept_no(corp_code: str, year: int, api_key: str | None = None) -> dict:
    ...
    3/6/9월 결산법인 대응: 구간을 year~year+3 으로 확장.
    [기재정정] / [첨부정정] 등 정정 보고서가 있으면 가장 최근 접수분을 사용.
    ...
    params = {
        ...
        "corp_code": corp_code,
        "pblntf_ty": "A",                       # 정기공시
        "bgn_de": f"{year}0101",                # 3/6/9월 결산법인: year 초부터
        "end_de": f"{year + 3}1231",            # 최대 year+3 말까지 확장
        "page_count": "100",
    }
    ...
    def _match_year(report_nm: str, yr: int) -> bool:
        ...
        # [첨부정정] prefix 대응: "[첨부정정]사업보고서 (2024.01)" 형태도 허용
        # 연도 문자열이 보고서명 어딘가에 포함되면 통과
        return yr_s in nm
