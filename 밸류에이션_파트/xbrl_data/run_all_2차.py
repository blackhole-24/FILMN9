...
# 처리 대상에서 제외할 키워드 (우선주, 스펙 등 사업보고서 불필요 종목)
SKIP_KEYWORDS = [
    "우선주", "1우", "2우", "3우", "우B", "우C",
    "SPAC", "스팩", "기업인수목적",
]
...
def _should_skip(name: str) -> bool:
    """종목명에 skip 키워드가 포함되면 True 반환."""
    for kw in SKIP_KEYWORDS:
        if kw in name:
            return True
    return False

def run_automation(csv_file="failed_stock_names.csv", target_year=2025, out_dir="./output_json"):
    # 1. 결과물을 저장할 폴더 생성
    ...
    for index, company in enumerate(company_list, start=1):
        ...
        if _should_skip(company):
            print(f"  [{index}/{total_count}] '{company}' → ⏭️  skip (우선주/스펙 제외)")
    ...
    print(f"총 대상: {total_count}건 | 처리: {total_count - len(skipped_companies)}건 "
          f"| skip: {len(skipped_companies)}건 | 실패: {len(failed_companies)}건")
    print(f"저장 폴더: {out_dir}")

    if skipped_companies:
        print(f"\n[skip 종목 목록 — 우선주/스펙 {len(skipped_companies)}건]")
...
if __name__ == "__main__":
    ...
    run_automation(csv_file="failed_stock_names.csv", target_year=2025, out_dir="./xbrl_results")