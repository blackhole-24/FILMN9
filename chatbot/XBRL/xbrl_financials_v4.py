...
def get_rcept_no(corp_code: str, year: int, api_key: str | None = None) -> dict:
    ...
    # 가장 최근 접수분 우선 — 단, '첨부정정'(첨부서류만 교체)은 fnlttXbrl 엔드포인트에
    # XBRL 인스턴스가 없어 status=014 로 실패한다. 반면 원본 사업보고서와 '기재정정'(본문
    # 정정)은 XBRL 을 보유. 따라서 첨부정정을 후순위로 밀어 본문 보유 보고서를 우선 선택.
    #   정렬키 = (첨부정정 아님?, 접수일) 내림차순 → 첨부정정은 최후의 수단으로만 사용.
    def _sort_key(it: dict) -> tuple:
        nm = it.get("report_nm", "")
        is_attach_only = "첨부정정" in nm
        return (0 if is_attach_only else 1, it.get("rcept_dt", ""))
...
def get_financials(df: pd.DataFrame, year_cur: int, year_pfy: int | None = None,
    ...
    # ── NWC (당기/전기) ───────────────────────────────────────
    def _nwc(year_label):
        # 영업 운전자본 확장 — 매출채권·재고·매입채무 3개만 보던 협소 버전 개선.
        #  · 영업자산: (매출채권+기타채권) + 재고 + 선급금·선급비용
        #  · 영업부채: (매입채무+기타채무) + 선수금·계약부채
        #  '기타채권/채무 포함 태그(TradeAndOther…)'를 우선 채택해 기타 영업 운전자본까지 포착.
        ar = (g("TradeAndOtherCurrentReceivables",  "BS", year_label) or
              g("CurrentTradeReceivables",          "BS", year_label) or
              g("ShortTermTradeReceivable",         "BS", year_label) or 0)
        inv = g("Inventories", "BS", year_label) or 0
        prepaid = ((g("CurrentPrepayments",        "BS", year_label) or 0) +
                   (g("ShortTermPrepaidExpenses",  "BS", year_label) or 0) +
                   (g("ShortTermAdvancePayments",  "BS", year_label) or 0))
        ap = (g("TradeAndOtherCurrentPayables",                 "BS", year_label) or
              g("TradeAndOtherCurrentPayablesToTradeSuppliers", "BS", year_label) or
              g("ShortTermTradePayables",                       "BS", year_label) or 0)
        unearned = ((g("CurrentContractLiabilities", "BS", year_label) or 0) +
                    (g("CurrentAdvances",            "BS", year_label) or 0))
        oca = ar + inv + prepaid
        ocl = ap + unearned
        return {
            "매출채권등": ar, "재고자산": inv, "선급금등": prepaid,
            "매입채무등": ap, "선수금등": unearned,
            "NWC": oca - ocl,
        }
    ...
    # ── CAPEX (현금흐름표, 유형 + 무형 자산 취득) ──────────────────────
    # ★무형자산취득 추가 — 바이오·제약·IT는 개발비/SW capex가 유형 못지않음
    #   (예: 셀트리온 무형 0.22조 > 유형 0.11조). 누락 시 capex 과소 → FCFF·적정주가 과대.
    capex_ppe = (g("PurchaseOfPropertyPlantAndEquipmentClassifiedAsInvestingActivities", "CF") or
                 g("AcquisitionOfPropertyPlantAndEquipmentClassifiedAsInvestingActivities", "CF") or 0)
    capex_intangible = (g("PurchaseOfIntangibleAssetsClassifiedAsInvestingActivities", "CF") or
                        g("AcquisitionOfIntangibleAssetsClassifiedAsInvestingActivities", "CF") or 0)
    capex = capex_ppe + capex_intangible   # 통상 양수(투자활동 유출). FCFF 에서 abs() 사용.
    ...
    return {
        ...
        "nwc_pfy":   nwc_pfy,
        "delta_nwc": delta_nwc,
        # CAPEX (현금흐름표, 유형+무형 자산 취득 합계)
        "capex":            capex,
        "capex_ppe":        capex_ppe,         # 유형자산취득
        "capex_intangible": capex_intangible,  # 무형자산취득 (★신규 반영)
        # IBD
        "ibd":        IBD,
