...
# ─────────────────────────────────────────────────────────────
# 지식 폴백 (RAG 미검색 시 LLM 일반지식 답변)
# ─────────────────────────────────────────────────────────────
# 검색 결과 0건이거나 회사를 특정 못 했을 때, 거부("찾을 수 없음") 대신 LLM 일반지식으로
# 답하되 'DART 근거 아님' 경고배지를 함께 표시(NO-MOCK 분리). 기본 ON(사용자 결정 ⓑ).
# 환각 방지: 일반지식 모드는 특정 회사 구체수치를 보고서처럼 단정하지 않도록 별도 프롬프트 사용.
ENABLE_KNOWLEDGE_FALLBACK = True
