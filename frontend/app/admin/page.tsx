...
/* ────────────────────────────────────────────────────────────────────────
   FINSIGHT 관리자/QA 페이지 (밸류에이션 · 챗봇)
   - 탭1 테스트 페이지: QA 관점 서비스 검증 (헬스 + 종목 밸류/챗봇 실측)
   - 탭2 관리자 페이지: 데이터 기준일 · 출처 · 테스트 환경 · AI 하네스(통제 원칙)
   - 평가위원이 보는 화면. 참고 디자인(데이터 원천 지도) 다크 테마 차용.
   ──────────────────────────────────────────────────────────────────────── */
...
export default function AdminPage() {
  const [tab, setTab] = React.useState<"test" | "admin">("admin");
  ...
  React.useEffect(() => {
    ...
    tfetch(`${CHAT_API}/health`, {}, 6000).then(setCbHealth).catch((e) => { setCbHealth({ status: "down" }); setCbErr(String(e).slice(0, 40)); });
  }, []);
  ...
  return (
    <div style={{ background: T.bg, color: T.text, minHeight: "100vh", fontFamily: "'Noto Sans KR', sans-serif", padding: "24px 32px" }}>
      <h1 style={{ fontSize: 26, color: T.accent, marginBottom: 2 }}>🛠 FINSIGHT 관리자 · QA 콘솔</h1>
      <p style={{ color: T.sub, fontSize: 13, marginBottom: 18 }}>
        밸류에이션 · AI 챗봇 — 데이터 기준일 / 출처 / 테스트 환경 / AI 하네스 ·
        포트 {mono("3000·8090·8800")} · 생성 {meta?.generated_at || "…"}
      </p>

      {/* 탭 */}
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        {([["admin", "📋 관리자 페이지"], ["test", "🧪 테스트 페이지"]] as const).map(([id, label]) => (
          <button key={id} onClick={() => setTab(id)}
            style={{
              padding: "9px 18px", borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: "pointer",
              border: `1px solid ${tab === id ? T.accent : T.border}`,
              background: tab === id ? T.accent : T.panel, color: tab === id ? T.bg : T.text,
            }}>{label}</button>
        ))}
      </div>

      {tab === "admin" ? <AdminTab v={v} co={co} cbChunks={cbChunks} cbDevice={cbDevice} cbGpu={cbGpu} /> :
        <TestTab beHealth={beHealth} cbHealth={cbHealth} cbErr={cbErr} v={v} />}

      <footer style={{ textAlign: "center", marginTop: 36, paddingTop: 18, borderTop: `1px dashed #475569`, color: "#64748B", fontSize: 11 }}>
        © FINSIGHT (FILMN9) · 관리자/QA 콘솔 · NO-MOCK(실데이터 100%) 원칙 · 평가위원 검토용
      </footer>
    </div>
  );
}
...
// ── 관리자 탭 ──────────────────────────────────────────────────────────────
function AdminTab({ v, co, cbChunks, cbDevice, cbGpu }: any) {
    ...
    <>
      <Section title="① 데이터 기준일 (평가에 사용된 데이터의 As-of)">
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 10 }}>
          <Card title={<>💰 밸류에이션 <Tag k="sqlite">valuation.db</Tag></>}>
          ...
          <Card title={<>📋 기업개요 <Tag k="sqlite">filmn9.db</Tag></>}>
            <KV k="주가(OHLCV)">{mono(dash(co.ohlcv_latest))} <span style={{ color: T.sub }}>({dash(co.ohlcv_rows)}행)</span></KV>
            <KV k="재무연도">{mono(`FY${dash(co.fiscal_years?.[0])}~${dash(co.fiscal_years?.[1])}`)}</KV>
            <KV k="기업정보">{mono(`${dash(co.company_info)} 종목`)}</KV>
            <KV k="재무상세(BS/IS)">{mono(`${dash(co.financial_detail_stocks)} 종목 / ${dash(co.financial_detail_rows)}행`)}</KV>
            <div style={{ marginTop: 6, fontSize: 11.5, color: T.amber, background: "#312E81", borderLeft: `3px solid ${T.accent}`, padding: "6px 10px", borderRadius: "0 4px 4px 0" }}>
              ※ 현재 데모 번들 — 재무상세·Sankey는 전체 데이터 적재 시 확장
            </div>
          </Card>
          <Card title={<>🤖 AI 챗봇 <Tag k="chroma">Chroma</Tag></>}>
      ...
      <Section title="② 데이터 출처 (Source)">
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(330px, 1fr))", gap: 10 }}>
          {SOURCES.map((s, i) => (
            <Card key={i}><div style={{ display: "flex", gap: 8, alignItems: "baseline" }}><Tag k={s.k}>SRC</Tag>
              <div><div style={{ fontWeight: 700, color: "#F1F5F9", fontSize: 13.5 }}>{s.name}</div>
                <div style={{ color: T.sub, fontSize: 12, marginTop: 2 }}>{s.desc}</div></div></div></Card>
          ))}
        </div>
      </Section>
...
function TestTab({ beHealth, cbHealth, cbErr, v }: any) {
  ...
  const runVal = async () => {
    ...
    try { const d = await tfetch(`${API}/api/valuation/${code.trim()}`, {}, 15000); setValRes(d); }
  ...
  const checks = valRes && !valRes._error ? [
    ["실데이터(is_mock=false)", valRes.is_mock === false],
    ...
    <>
      ...
      <Section title="🧪 밸류에이션 QA (종목 실측)">
        <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
          ...
          <span style={{ color: T.sub, fontSize: 12, alignSelf: "center" }}>데모: 009150 · 035420 · 090430 (그 외 다수 종목도 평가됨)</span>
        </div>
        {valRes?._error && <Card><span style={{ color: T.red }}>오류: {valRes._error}</span></Card>}
        {valRes && !valRes._error && (
          <Card title={<>결과 · {valRes.company?.name || code} {valRes.is_mock ? <Tag k="warn">미평가(준비중)</Tag> : <Tag k="ok">실데이터</Tag>}</>}>
            {valRes.is_mock ? <div style={{ color: T.amber, fontSize: 13 }}>이 종목은 아직 평가 전 → NO-MOCK 정책상 가짜 숫자 미표시 (정상 동작)</div> : (
              <>
      ...
      <Section title="🤖 챗봇 QA (RAG 실측)">
        <div style={{ fontSize: 12, color: T.amber, marginBottom: 8 }}>
          ⚠️ 현재 설정(reasoning='high' + 멀티쿼리)은 정확도↑·속도↓ — 첫 질의는 수십초~수분 소요될 수 있습니다.
        </div>
        <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
        ...
        {chat?._error && <Card><span style={{ color: T.red }}>오류: {chat._error}</span></Card>}
        {chat && !chat._error && (
          <Card title={<>답변 {Array.isArray(chat.sources) ? <Tag k="ok">출처 {chat.sources.length}건</Tag> : null}</>}>
