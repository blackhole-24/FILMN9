# -*- coding: utf-8 -*-
"""20f: DART 2025 1분기(reprt_code=11013) → financials 적재.
- 목적: 2026 1분기 재무 하이라이트의 YoY(전년동기比) 비교 대상 확보.
- 20d와 동일한 검증된 방식(DART fnlttSinglAcntAll 원raw→adaptive), 계정명 변형 처리 동일.
- fiscal_year=2025, reprt_code='11013'. (2025 연간 11011과 별개 행)"""
...
print(f"대상 {len(targets)}종 · DART 2025 1분기 수집 시작")
    ...
    if lst:
        ...
        if rev is not None or v.get("op") is not None:
            recs.append((code, 2025, f(rev), f(v.get("op")), f(v.get("ni")), f(a),
                         f(li), f(eq), dr, "백만원", "DART 2025 1분기(11013)", NOW, NOW, "11013"))
    time.sleep(0.06)
    if i % 300 == 0:
...
sq.execute("DELETE FROM financials WHERE fiscal_year=2025 AND reprt_code='11013'")
...
n = pg.execute("SELECT COUNT(*) FROM financials WHERE fiscal_year=2025 AND reprt_code='11013'").fetchone()[0]
ni = pg.execute("SELECT COUNT(*) FROM financials WHERE fiscal_year=2025 AND reprt_code='11013' AND net_income IS NOT NULL").fetchone()[0]
chk = pg.execute("SELECT revenue,op_income,net_income FROM financials WHERE stock_code='005930' AND fiscal_year=2025 AND reprt_code='11013'").fetchone()
pg.close()
def _fmt(x):
    return f"{x:,}" if x is not None else "None"
print(f"\n=== 완료: 2025 1분기 {n}종 적재 ({time.time()-t0:.0f}s) ===")
