# -*- coding: utf-8 -*-
"""20d-②: DART 2025 사업보고서(reprt_code=11011) → financials 재적재.
- B-1(financial_detail 경유) 대형주 174종 스케일 1000배 버그 수정용.
- 20d와 동일한 검증된 방식: DART API fnlttSinglAcntAll 직접 수집(원 raw → adaptive 스케일).
- 연결(CFS) 우선·없으면 별도(OFS), 접두사/계정명 변형 정규화, SQLite+Postgres.
- fiscal_year=2025, reprt_code='11011'."""
...
print(f"대상 {len(targets)}종 · DART 2025 사업보고서 수집 시작")
...
for i, code in enumerate(targets, 1):
    ...
    if lst:
        ...
        if rev is not None or v.get("op") is not None:
            recs.append((code, 2025, f(rev), f(v.get("op")), f(v.get("ni")), f(a),
                         f(li), f(eq), dr, "백만원", "DART 2025 사업보고서(11011)", NOW, NOW, "11011"))
    time.sleep(0.06)
    if i % 300 == 0:
...
sq.execute("DELETE FROM financials WHERE fiscal_year=2025 AND reprt_code='11011'")
...
pg.execute("DELETE FROM financials WHERE fiscal_year=2025 AND reprt_code='11011'")
with pg.cursor() as cur:
    cur.executemany(f"INSERT INTO financials ({cols}) VALUES ({','.join(['%s']*14)})", recs)
n = pg.execute("SELECT COUNT(*) FROM financials WHERE fiscal_year=2025 AND reprt_code='11011'").fetchone()[0]
ni = pg.execute("SELECT COUNT(*) FROM financials WHERE fiscal_year=2025 AND reprt_code='11011' AND net_income IS NOT NULL").fetchone()[0]
chk = pg.execute("SELECT revenue,op_income,net_income FROM financials WHERE stock_code='005930' AND fiscal_year=2025").fetchone()
pg.close()
def _fmt(x):
    return f"{x:,}" if x is not None else "None"
print(f"\n=== 완료: 2025 사업보고서 {n}종 적재 ({time.time()-t0:.0f}s) ===")
print(f"삼성전자(005930): 매출={_fmt(chk[0])} 영업익={_fmt(chk[1])} 순익={_fmt(chk[2])} 백만원" if chk else "삼성전자 없음")
print(f"순이익 채워진 종목: {ni}종")
